<script>
	...
	async function loadAvailableOrders() {
		...
		try {
			orders = await driverService.getAvailableOrders();
	...
	async function showOrderDetails(order) {
		console.log('showOrderDetails called with order:', order);
		try {
			console.log('Fetching order details for ID:', order.id);
			selectedOrder = await driverService.getOrderDetails(order.id);
			console.log('Order details received:', selectedOrder);
			showOrderModal = true;
			console.log('Modal should now be visible, showOrderModal:', showOrderModal);
		} catch (err) {
	...
	async function takeOrder() {
		if (!selectedOrder || isTakingOrder) return;
		
		isTakingOrder = true;
		
		try {
			await driverService.takeOrder(selectedOrder.id, user.id);
			...
			alert(currentLang === 'ar' ? 'تم أخذ الطلب بنجاح!' : 'Order taken successfully!');
			
		} catch (err) {
			console.error('Failed to take order:', err);
			alert(currentLang === 'ar' ? 'فشل في أخذ الطلب: ' + err.message : 'Failed to take order: ' + err.message);
		} finally {
			isTakingOrder = false;
		}
	}
...
<svelte:head>
	<title>{currentLang === 'ar' ? 'الطلبات المتاحة للتوصيل - مصنع باتريوت للزجاج' : 'Available Orders - Patriot Glass Factory'}</title>
</svelte:head>

<div class="available-orders-page" data-theme={currentTheme}>
	<div class="container">
		...
		{:else if error}
			<div class="error-state">
				...
				<button class="btn btn-primary" on:click={loadAvailableOrders}>
					{#if currentLang === 'ar'}
						إعادة المحاولة
					{:else}
						Retry
		...
		{:else if orders.length === 0}
			<div class="empty-state">
				...
				<p>
					{#if currentLang === 'ar'}
						لا توجد طلبات مكتملة متاحة للتوصيل حالياً
					{:else}
						There are no completed orders available for delivery at the moment
					{/if}
				</p>
			</div>
		{:else}
				...
				{#each orders as order}
					<div class="order-card">
						...
						<div class="order-actions">
							<button class="btn btn-primary take-order-btn" on:click|stopPropagation={() => {
...
{#if showOrderModal && selectedOrder}
	<div class="modal-overlay" on:click={closeModal} style="z-index: 9999; background: rgba(0, 0, 0, 0.8);">
		<div class="modal-content" on:click|stopPropagation>
					...
					<div class="info-section">
						...
						<div class="info-grid">
							...
							<div class="info-item">
								...
								<span class="info-value status-completed">
									{#if currentLang === 'ar'}
										مكتمل
									{:else}
										Completed
			...
			<div class="modal-actions">
				<button class="btn btn-secondary" on:click={closeModal} disabled={isTakingOrder}>
					{#if currentLang === 'ar'}
						إلغاء
					{:else}
						Cancel
				...
				<button class="btn btn-primary" on:click={takeOrder} disabled={isTakingOrder}>
					{#if isTakingOrder}
						{#if currentLang === 'ar'}
							جاري أخذ الطلب...
						{:else}
